...
{
    public class GroupPEController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            FirebaseResponse response = infa.Get("GroupPE_Нижняя_Первая/");
        ...
        public ActionResult IndexTop()
        {
            infa = new FireSharp.FirebaseClient(config);
            //var link = groupMS;
            FirebaseResponse response = infa.Get("GroupPE_Верхняя_Первая/");
            //FirebaseResponse response = infa.Get(data.dayOfWeek + " / " + data.time);
            dynamic data = JsonConvert.DeserializeObject<dynamic>(response.Body);
            var list = new List<GroupMSModel>();
            foreach (var item in data)
        ...
        public ActionResult Index_Second()
        {
            infa = new FireSharp.FirebaseClient(config);
            //var link = groupMS;
            FirebaseResponse response = infa.Get("GroupPE_Нижняя_Вторая/");
            //FirebaseResponse response = infa.Get(data.dayOfWeek + " / " + data.time);
            dynamic data = JsonConvert.DeserializeObject<dynamic>(response.Body);
            var list = new List<GroupMSModel>();
            foreach (var item in data)
        ...
        public ActionResult Index_Top_Second()
        {
            infa = new FireSharp.FirebaseClient(config);
            //var link = groupMS;
            FirebaseResponse response = infa.Get("GroupPE_Верхняя_Вторая/");
            //FirebaseResponse response = infa.Get(data.dayOfWeek + " / " + data.time);
            dynamic data = JsonConvert.DeserializeObject<dynamic>(response.Body);
            var list = new List<GroupMSModel>();
            foreach (var item in data)
        ...
        private void AddDataToFirebase(GroupMSModel groupMS)
        {
            ...
            if (Convert.ToString(data.tipeOfWeek) == "Нижняя")
            {
                if (Convert.ToString(data.subgroup) == "Первая")
                    link = "GroupPE_Нижняя_Первая/";
                else
                    link = "GroupPE_Нижняя_Вторая/";
            }
            else
            {
                if (Convert.ToString(data.subgroup) == "Первая")
                    link = "GroupPE_Верхняя_Первая/";
                else
                    link = "GroupPE_Верхняя_Вторая/";
            }
